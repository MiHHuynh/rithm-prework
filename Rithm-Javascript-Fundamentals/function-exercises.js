...
function multipleLetterCount(str) {
	...
	for (var i = 0; i < str.length; i++) {
		/* 
		loop through string
		hits first letter --> count[first letter] = 1
		hits second letter --> count[second letter] = 1, etc.
		if letter already exists in object, increment
		else, count[current letter] = 1.
		*/
		if (str[i] in obj) {
			obj[str[i]] += 1;
		} else {
...
function rockPaperScissors() {
	...
	var userChoice = prompt("Type rock, paper, or scissors:").toLowerCase();
	var computerChoice = choices[(Math.floor((Math.random() * 3) + 1)) - 1];
	var results = ["You win.", "You lose.", "It's a tie."];
	var endMessage = "You played " + userChoice + ". The computer played " + computerChoice + ". ";
	if (userChoice === computerChoice) {
		return endMessage + results[2];
	}
	if (userChoice === choices[0]) { //if user played ROCK
		if (computerChoice === choices[1]) { //computer plays PAPER
			return endMessage + results[1];
		}
		if (computerChoice === choices[2]) { //computer plays SCISSORS
			return endMessage + results[0];
		}
	}
	if (userChoice === choices[1]) { //if user played PAPER
		if (computerChoice === choices[0]) { //computer plays ROCK
			return endMessage + results[0];
		}
		if (computerChoice === choices[2]) { //computer playes SCISSORS
			return endMessage + results[1];
		}

	}
	if (userChoice === choices[2]) { //if user played SCISSORS
		if (computerChoice === choices[0]) { //computer plays ROCK
			return endMessage + results[1];
		}
		if (computerChoice === choices[1]) { //computer plays PAPER
